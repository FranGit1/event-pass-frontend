...
export const FromFileUploadCard = (props: IFromFileUploadCardProps) => {
  ...
  const truncatedFileName = file.name.length > 15 ? file.name.substring(0, 8) + '...' : file.name;

  return (
    <div css={[tw`flex flex-col mb-2 items-start bg-gray-600 px-4 py-2 rounded-lg w-[20rem] m-4`, props.containerCss]}>
      <div tw="flex flex-row items-center pb-2">
        <RxCross2 onClick={onFileRemove} tw="w-6 h-6 cursor-pointer mr-2" />
        <Typography.Body containerCss={[tw`flex-1 truncate mr-2 `]}>{truncatedFileName}</Typography.Body>
        <Typography.Body containerCss={[tw`mr-2`]}>{formattedSize} kB</Typography.Body>
        <Download />
      </div>
      <div tw="w-[8.875rem] h-[7.94775rem] bg-gray-700"></div>
    </div>
  );
};
